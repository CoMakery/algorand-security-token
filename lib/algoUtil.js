...
exports.deploySecurityToken = async (algodClient, account) => {
    ...
    let decimals = this.bigIntToUint8Array('8')
    ...
    let approvalProgram = await this.compileProgram(algodClient, approvalTeal)
    let clearProgram = await this.compileProgram(algodClient, clearTeal)
    let localInts = 3
    let localBytes = 0
    let globalInts = 4
    let globalBytes = 1
